...
linkModules ::
  ...
  -> IO ()
linkModules dest src  = (\cont -> runAnyContT' return cont) $ do
...
withModuleFromLLVMAssembly (Context c) s f = (\cont -> runAnyContT' return cont) $ do
...
writeLLVMAssemblyToFile (File path) m = (\cont -> runAnyContT' return cont) $ do
  m' <- readModule m
...
withModuleFromBitcode (Context c) b f = (\cont -> runAnyContT' return cont) $ do
...
writeBitcodeToFile (File path) m = (\cont -> runAnyContT' return cont) $ do
  m' <- readModule m
...
targetMachineEmit fileType (TargetMachine tm) m os = (\cont -> runAnyContT' return cont) $ do
  msgPtr <- alloca
  m' <- readModule m
...
emitToFile fileType tm (File path) m = (\cont -> runAnyContT' return cont) $ do
...
emitToByteString fileType tm m = (\cont -> runAnyContT' return cont) $ do
