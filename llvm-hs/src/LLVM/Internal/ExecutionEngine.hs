...
removeModule e m = (\cont -> runAnyContT' return cont) $ do
  d0 <- alloca
  d1 <- alloca
...
instance ExecutionEngine (Ptr FFI.ExecutionEngine) (FunPtr ()) where
  ...
  getFunction (ExecutableModule e m) (A.Name name) = (\cont -> runAnyContT' return cont) $ do
    name <- encodeM name
...
withExecutionEngine :: 
  ...
  IO a
withExecutionEngine c m createEngine f = (\cont -> runAnyContT' return cont) $ do
